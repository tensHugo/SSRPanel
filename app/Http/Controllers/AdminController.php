...
class AdminController extends Controller
{
    ...
    public function userBalanceLogList(Request $request)
    {
     ...
     // 充值记录列表
    public function qypayList(Request $request)
    {
        $username = trim($request->get('username'));
        $pay_id = trim($request->get('pay_id'));
		$pay_no = trim($request->get('pay_no'));

        $query = QypayOrder::query()->with(['user'])->orderBy('id','created_at');
        ...
        return Response::view('admin/qypayList', $view);
    }
