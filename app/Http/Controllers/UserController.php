...
use App\Http\Models\QypayOrder;
...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($nodeList as &$node) {
            ...
            $ssr_str .= '&group=' . $this->base64url_encode('飞天猪网络加速平台');
            $ssr_str .= '&udpport=0';
            $ssr_str .= '&uot=0';
            ...
            $ss_str = $this->base64url_encode($ss_str) . '#' . '飞天猪网络加速平台';
            $ss_scheme = 'ss://' . $ss_str;
   ...
   // 充值记录
    public function qyorderList(Request $request)
    {
        $user = $request->session()->get('user');

        $qyorderList = QypayOrder::query()->where('uid', $user['id'])->orderBy('id','created_at')->paginate(10)->appends($request->except('page'));
        if (!$qyorderList->isEmpty()) {
            foreach ($qyorderList as & $order) {
                $order->totalOriginalPrice = $order->totalOriginalPrice / 100;
                $order->totalPrice = $order->totalPrice / 100;
            }
        }

        $view['qyorderList'] = $qyorderList;

        return Response::view('user/qyorderList', $view);
    }
    ...
    public function activeUser(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            $request->session()->flash('successMsg', '邮件已发送，请查看邮箱。邮件可能会被拦截，请注意查看垃圾箱');
    ...
    public function active(Request $request, $token)
    {
        ...
        $request->session()->flash('successMsg', '账号激活成功');
         ...
         //邀请人返流量
         $user1 = User::query()->where('id',$verify->user_id)->first();
         if($user1->referral_uid!=0){
            User::query()->where('id',$user1->referral_uid)->increment('transfer_enable',self::$config['referral_traffic'] * 1048576);
         }
    ...
    public function addOrder(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
          ...
          //验证0元商品是否重复购买
          $jilu = Order::query()->where('user_id',$user['id'])->where('totalPrice',0)->first();
          if($jilu!=null and $goods->price == 0){
             return Response::json(['status' => 'fail', 'data' => '', 'message' => '购买失败：0元商品只能购买一次']);
          }
          
           //验证当前套餐是否到期，到期后才可购买套餐
		  $user = User::query()->where('id', $user['id'])->first();
		  if($user['enable'] == 1){
		  	if($goods->price > 500){
		  	    $jilu = Order::query()->where('user_id',$user['id'])->where('totalPrice','>',500)->where('is_expire',0)->first();
                if($jilu!=null){
          	        return Response::json(['status' => 'fail', 'data' => '', 'message' => '购买失败：当前还有未到期套餐，请到期后再次购买。当前套餐到期日：'.$jilu->expire_at]);
                }
		    }
		  }
            ...
            try {
                ...
                if ($goods->type == 2 and $goods->price > 0) {
                    User::query()->where('id', $user->id)->update(['transfer_enable' => 0,'u' => 0,'d' => 0]);
               ...
               //改变用户的等级 如果商品等级是0则不改变
              if($goods->level > 0 and $goods->type == 2){
                 User::query()->where('id', $user->id)->update(['level'=> $goods->level]);
              }
              
               //改变用户付费类型
               if($goods->price > 500){
                  $payType = 0;
                if($goods->days >= 30 and $goods->days < 182){
                   $payType = 1;
                 }elseif($goods->days >= 182 and $goods->days < 365){
                   $payType = 2;
                 }elseif($goods->days >= 365){
                   $payType = 3;
                 }
                 User::query()->where('id', $user->id)->update(['pay_way'=> $payType]);
               }
                ...
                if ($goods->type == 2 and $goods->price > 0) {
                    ...
                    //购买有效期，从购买日期起算
                    User::query()->where('id', $user->id)->update(['enable_time' => date('Y-m-d'),'traffic_reset_day' => $traffic_reset_day, 'expire_time' => date('Y-m-d', strtotime("+" . $goods->days . " days", time())), 'enable' => 1]);
                } elseif($goods->type == 2 and $goods->price == 0) {
                    User::query()->where('id', $user->id)->update(['enable_time' => date('Y-m-d'),'expire_time' => date('Y-m-d', strtotime("+" . $goods->days . " days", time())), 'enable' => 1]);
                }else{
                    // 将商品的有效期和流量自动重置日期加到账号上
    ...
    public function charge(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $chong = $coupon->amount / 100;
            $yue = $user->balance / 100;
            $url='https://sc.ftqq.com/SCU19866T6e14e778622c7ed362c46dcd329a9c225a5597266b473.send?text=飞天猪充值提醒&desp=用户名：'.$user->username.'%0D%0A%0D%0A'.'充值金额：'.$chong.'%0D%0A%0D%0A'.'现余额：'.$yue;
            $url1='https://sc.ftqq.com/SCU20372Tf05cc572b14cda0f98f278c5002e852b5a60b0334042d.send?text=飞天猪充值提醒&desp=用户名：'.$user->username.'%0D%0A%0D%0A'.'充值金额：'.$chong.'%0D%0A%0D%0A'.'现余额：'.$yue;
   ...
   public function qypay_send(Request $request){
    	$shop_id=3473;
   	    $key = "4aa390add3f240f08b740c08f9345a61";
        $pid = $request->get("pid");
        $url = $request->get("url");
        $user_id = $request->get("user_id");
	    $bank_Type = $request->get("bank_type");          //充值渠道
	    $bank_payMoney = $request->get("totalAmount");    //充值金额
		$orderid="ftz" . time() . time();                  //商户的订单ID，【请根据实际情况修改】
        $callbackurl="http://user.fiypig.org/api/qypay";        //商户的回掉地址，【请根据实际情况修改】
        $gofalse=$url;                    //订单二维码失效，需要重新创建订单时，跳到该页
        $gotrue=$url;                         //支付成功后，跳到此页面
      ...
      //检测是否有未支付订单
        $dingdan = QypayOrder::query()->where('uid',$user_id)->where('status',2)->first();
		if($dingdan != null && $pid == ""){
			return "<script>alert('您有待支付的充值记录，请点击支付按钮直接支付或点击删除按钮作废订单。请到充值记录中查看');window.location.href='/user/qyorderList';</script>";
		}
		...
		if($pid == ""){
			//写入订单信息
            $QypayOrder = new QypayOrder();
		    $QypayOrder->uid = $user['id'];
		    $QypayOrder->pay_id = $orderid;
		    $QypayOrder->money = $bank_payMoney;
		    $QypayOrder->type = $bank_Type;
		    $QypayOrder->pay_tag = '';
		    $QypayOrder->creat_time = time();
		    $QypayOrder->save();
		}

        $charset="utf-8";                                              //字符集编码方式
        $token="";                                                 //自定义传过来的值 千应平台会返回原值
   ...
   public function qypay(Request $request){
   	    $key = "4aa390add3f240f08b740c08f9345a61"; 
	    $orderid = $request->get("oid");
	    $status = $request->get("status");
		$money = $request->get("m1");
		$sign = $request->get("sign");
		$time = $request->get("time");
		$token = $request->get("token");
		$orderidMy = $request->get("oidMy");    //千应录入时产生流水号，建议保存以供查单使用
		$orderidPay = $request->get("oidPay");  //收款方的订单号（例如支付宝交易号）;
		$msg = $request->get("msg");
		...
		if(strcasecmp($sign,$paramMd5)==0){
			if($status == "1" || $status == "5" || $status == "6"){
				DB::beginTransaction();
				try{
					$QypayOrder = QypayOrder::query()->where('pay_id',$orderid)->first();
					$user = User::query()->where('id',$QypayOrder->uid)->first();
					
					//更新订单信息
					$QypayOrder->price = $money;
					$QypayOrder->pay_no = $orderidMy;
					$QypayOrder->param = $token;
					$QypayOrder->pay_time = $time;
					$QypayOrder->pay_tag = $orderidPay;
					$QypayOrder->status = 1;
					$QypayOrder->up_time = date('Y-m-d H:i:s');
					$QypayOrder->save();
					
					// 写入日志
					if($QypayOrder->type == 101){
						$desc = '用户手动充值 - [支付宝充值]';
					}else if($QypayOrder->type == 102){
						$desc = '用户手动充值 - [微信充值]';
					}
					$money = $money * 100;
                    $log = new UserBalanceLog();
                    ...
                    $user->save();
					DB::commit();
					return Response::json(['status' => 'success', 'data' => $paramMd5, 'message' => '充值成功']);
				} catch (\Exception $e){
                    Log::error($e->getMessage());
                    DB::rollBack();
                    return Response::json(['status' => 'fail', 'data' => '', 'message' => '充值失败']);
			...
			return Response::json(['status' => '0', 'data' => $paramMd5, 'message' => '签名验证失败']);
		}  
   	
   }
  
  //作废充值订单
  public function qyOrderDel(Request $request){
   	   $oid = $request->get("oid");
	   $user = $request->session()->get('user');
	   QypayOrder::query()->where('uid', $user['id'])->where('id', $oid)->update(['status' => 4]);
	   return Response::json(['status' => '1', 'message' => '作废成功']);
   }
