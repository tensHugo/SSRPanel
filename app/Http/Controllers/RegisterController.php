...
class RegisterController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            // 创建新用户
            $transfer_enable = $referral_uid ? (self::$config['default_traffic'] + self::$config['referral_traffic']) * 1048576 : self::$config['default_traffic'] * 1048576; //获得流量
            $user = new User();
            ...
            //邀请人返流量  转移到激活函数
            //if($referral_uid!=0){
              // User::query()->where('id',$referral_uid)->increment('transfer_enable',self::$config['referral_traffic'] * 1048576);
            //}
            ...
            if (self::$config['is_active_register']) {
                ...
                $request->session()->flash('regSuccessMsg', '注册成功：激活邮件已发送，请查看邮箱。邮件可能会被拦截，请注意查看垃圾箱');
            } else {
